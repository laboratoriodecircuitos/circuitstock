...
import { updateLocation } from "@/app/localizacoes/actions";
import { prisma } from "@/lib/prisma";

type EditarLocalizacaoPageProps = {
  params: Promise<{
    id: string;
  }>;
...
export default async function EditarLocalizacaoPage({
  params,
  searchParams,
}: EditarLocalizacaoPageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams]);
  const location = await getEditPageData(id);
  ...
  return (
    <section className="mx-auto max-w-5xl">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            Atualize nome e descricao da localizacao sem alterar os componentes
            vinculados.
          </p>
        </div>
      ...
      >
        <input name="id" type="hidden" defaultValue={location.id} />
        ...
        <div className="flex flex-col gap-1 border-b border-slate-200 pb-4">
          ...
          <p className="text-sm text-slate-600">
            Esta localizacao possui{" "}
            {location._count.components.toLocaleString("pt-BR")} componente
            {location._count.components === 1 ? "" : "s"} vinculado
            {location._count.components === 1 ? "" : "s"}.
          </p>
        </div>
